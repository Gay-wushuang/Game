...
public partial class TrainingArenaSmoke : Node
{
    ...
    private async Task Run()
    {
        ...
        var aiCounter = new CardInstance(counterDef, "ai"); aiDeck.Hand.Add(aiCounter); Check(aiDeck.SetPassive(aiCounter) && enemy.SetPassive(aiCounter), "AI被动锦囊盖放失败"); SetPrivate(arena, "_ap", 3); var steal = new CardInstance(stealDef); deck.Hand.Add(steal); arena.RefreshAll(); await Frame(); var tile = arena.GetNode<HandFan>("%Hand").GetChildren().OfType<CardTile>().First(t => t.Card == steal); tile.EmitSignal(Button.SignalName.Pressed); await Frame(); tile.EmitSignal(Button.SignalName.Pressed); await Delay(3.0); Check(deck.DiscardPile.Contains(steal), "拿来主义未进入弃牌堆"); Check(aiDeck.DiscardPile.Contains(aiCounter) && enemy.PassiveCard == null, "我觉得不行必须从英雄槽翻开、抵消并进入弃牌堆");
