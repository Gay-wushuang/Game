...
public partial class TrainingArena : Control
{
    ...
    public void ResetTraining()
    {
        ...
        foreach (var s in _allies) { s.ClearPassive(); s.SetUnit(null); }
        for (var i = 0; i < _enemies.Count; i++) { _enemies[i].ClearPassive(); s_set(_enemies[i], N<CheckButton>("DummyMode").ButtonPressed && i < content.monsters.Count ? FromMonster(content.monsters[i]) : null); }
    ...
    public void SelectHero(HeroCardInstance hero) { if (_battle.IsFinished) return; if (_ap < 1) { _status.Text = "行动点不足：部署英雄需要1点"; return; } _pendingHero = hero; _pendingCard = null; N<AcceptDialog>("HeroBagDialog").Hide(); _status.Text = $"部署 {hero.Definition.display_name}：点击一个我方空位"; }
    ...
    public async Task ConfirmAttack() { if (_battle.IsFinished) return; if (_allyIndex < 0 || _enemyIndex < 0 || _ap <= 0) return; var a = _allies[_allyIndex].Unit!; var d = _enemies[_enemyIndex].Unit!; var counter = BattleRules.CalculateRetaliation(a, d); var damage = BattleRules.CalculateAttackValue(a, d); await AnimateAttack(_allies[_allyIndex], _enemies[_enemyIndex]); if (await TriggerPassive(_enemies, _aiDeck, "BEFORE_DAMAGE")) damage = 0; d.Hp = Math.Max(0, d.Hp - damage); if (a.Id == "hero_role_2" && a.SkillTurns > 0) counter = Mathf.RoundToInt(counter * (a.Star >= 5 ? 1.2f : 1.4f)); if (await TriggerPassive(_allies, _deck, "BEFORE_DAMAGE", new PassiveEventContext { EventKey = "BEFORE_DAMAGE", AttackTarget = a, AttackTargetSlot = _allyIndex })) counter = 0; ApplyDamageToAlly(a, counter); GainExp(a, 2); _ap--;
    ...
    private async Task UseCard(UnitSlot slot)
    {
        ...
        _ap -= cost; _deck.Discard(card); _pendingCard = null; _pendingCardTarget = -1;
        // 打牌后检查 HAND_EMPTY
    ...
    private async Task UseEnemyCard(UnitSlot slot)
    {
        ...
        _ap -= cost; _deck.Discard(card); AddLog($"[color=dd99ff]锦囊[/color] 「{definition.display_name}」对 {unit.Name} 生效。");
    ...
    private async Task UseNoTargetCard(CardInstance card)
    {
        ...
        if (definition.logic_mode == "LUA")
        {
            ...
            if (definition.handler_key == "PREPAY_AND_DISCARD") _ap = _battle.PlayerActionPoints; else _ap -= EffectiveCost(card); _deck.Discard(card); AddLog($"[color=dd99ff]Lua锦囊[/color] 「{definition.display_name}」已在沙盒中结算。");
            // 打牌后检查 HAND_EMPTY
            if (_deck.Hand.Count == 0) await TriggerPassive(_enemies, _aiDeck, "HAND_EMPTY", new PassiveEventContext { EventKey = "HAND_EMPTY", SubjectOwnerId = "player" });
            // 打牌消耗 AP 后检查 ACTION_POINTS_ZERO
            if (_ap == 0) await CheckActionPointsZero("player");
        ...
        if (_cardResolver.CanResolveBuiltin(definition.handler_key))
        {
            ...
            _ap -= EffectiveCost(card); _deck.Discard(card); AddLog($"[color=dd99ff]内置锦囊[/color] 「{definition.display_name}」已结算。");
            // 打牌后检查 HAND_EMPTY
            if (_deck.Hand.Count == 0) await TriggerPassive(_enemies, _aiDeck, "HAND_EMPTY", new PassiveEventContext { EventKey = "HAND_EMPTY", SubjectOwnerId = "player" });
            // 打牌消耗 AP 后检查 ACTION_POINTS_ZERO
            if (_ap == 0) await CheckActionPointsZero("player");
        ...
        _ap -= EffectiveCost(card); _deck.Discard(card); AddLog($"[color=dd99ff]锦囊[/color] 「{definition.display_name}」已结算。");
        // 打牌后检查 HAND_EMPTY
        if (_deck.Hand.Count == 0) await TriggerPassive(_enemies, _aiDeck, "HAND_EMPTY", new PassiveEventContext { EventKey = "HAND_EMPTY", SubjectOwnerId = "player" });
        // 打牌消耗 AP 后检查 ACTION_POINTS_ZERO
        if (_ap == 0) await CheckActionPointsZero("player");
    ...
    private void PlacePassive(UnitSlot slot)
    {
        ...
        if (slot.PassiveCard != null) { _status.Text = "该英雄槽已经设置了一张被动锦囊"; return; }
        
        // 先验证所有条件，再提交状态
        if (!_battle.TryPlacePassive("player", slot.SlotIndex, card, out var error)) { _status.Text = error; return; }
        if (!_deck.SetPassive(card)) { _battle.RemovePassive(card); _status.Text = "被动锦囊盖放失败"; return; }
        if (!slot.SetPassive(card)) { _battle.RemovePassive(card); _deck.DiscardPlaced(card); _status.Text = "被动锦囊盖放失败"; return; }
        
        _ap -= card.CurrentCost(); 
        AddLog($"[color=99bbff]伏牌[/color] 玩家在 {slot.SlotIndex + 1} 号英雄槽背面设置了1张被动锦囊。"); 
        _pendingCard = null; _pendingCardTarget = -1; _hand.SetSelected(-1); 
        _status.Text = "被动锦囊已背面盖放在英雄槽"; 
        RefreshAll();
    }
    ...
    private async Task EndTurn() { if (_battle.IsFinished) return; await TriggerPassive(_allies, _deck, "ALLY_TURN_ENDED"); await EnemyPhase(); if (_battle.IsFinished) return; _battle.AdvanceTurn(); _turn = _battle.Turn; _ap = 3 + _battle.PlayerNextTurnBonus; _battle.PlayerNextTurnBonus = 0; _battle.PlayerActionPoints = _ap; await TriggerPassive(_allies, _deck, "ALLY_BATTLE_PHASE_STARTED"); TickStatuses(); foreach (var s in _allies.Where(s => s.Unit is { Alive: true, Id: "hero_role_3" })) { _ap++; if (s.Unit!.Star >= 5) s.Unit.FreeSelfCards = 2; } _battle.PlayerActionPoints = _ap;
        ...
        if (_ap == 0) await CheckActionPointsZero("player"); AssignFreeCard(); if (_deck.Hand.Count < 5) {
            ...
            if (blockingDraw.Count > 0)
            {
                ...
                {
                    var slot = _enemies.FirstOrDefault(c => c.SlotIndex == placed.SlotIndex);
                    var card = slot?.RemovePassive() ?? placed.Card;
                    card.FaceUp = true;
            ...
            {
                var drawResult = _deck.Draw(); AddLog($"[color=75d7ff]抽牌[/color] 抽到 {drawResult.Count} 张。");
    ...
    private async Task EnemyPhase()
    {
        ...
        {
            // 发布 BEFORE_DRAW 事件，让玩家方被动锦囊有机会阻止
            var aiBeforeDrawTriggered = await TriggerPassive(_allies, _deck, "BEFORE_DRAW", new PassiveEventContext { EventKey = "BEFORE_DRAW", SubjectOwnerId = "ai" });
            if (!aiBeforeDrawTriggered)
            {
                var aiDrawResult = _aiDeck.Draw();
                // 发布 AFTER_DRAW 和 HAND_EMPTY 事件
                await TriggerPassive(_allies, _deck, "AFTER_DRAW", new PassiveEventContext { EventKey = "AFTER_DRAW", SubjectOwnerId = "ai" });
                if (_aiDeck.Hand.Count == 0)
                    await TriggerPassive(_allies, _deck, "HAND_EMPTY", new PassiveEventContext { EventKey = "HAND_EMPTY", SubjectOwnerId = "ai" });
            }
        }
        AddLog($"[color=ff8888]AI回合[/color] 敌方行动结束，剩余行动点 {aiAp}。");
    }
    ...
    private async Task<bool> AiAttack()
    {
        ...
        await AnimateAttack(chosen.E, chosen.A); if (await TriggerPassive(_allies, _deck, "BEFORE_DAMAGE")) damage = 0; ApplyDamageToAlly(target, damage); attacker.Hp = Math.Max(0, attacker.Hp - counter); MarkDefeated(); AddLog($"[color=ff8888]AI攻击[/color] {attacker.Name} → {target.Name}：{BattleRules.GetRelation(attacker.Type, target.Type)}，伤害{damage}，受到反伤{counter}。"); chosen.E.Refresh(); chosen.A.Refresh(); return true;
    }
    private async Task<bool> AiUseCard()
    {
        if (_battle.IsFinished) return false;
        var playable = _aiDeck.Hand.Where(c => c.Definition.card_kind == CardDefinition.CardKind.Active || _enemies.Any(s => s.Unit?.Alive == true && s.PassiveCard == null)).ToList(); if (playable.Count == 0) return false; var card = playable[_battle.Random.Next(playable.Count)];
        if (card.Definition.card_kind == CardDefinition.CardKind.Passive) { var slots = _enemies.Where(s => s.Unit?.Alive == true && s.PassiveCard == null).ToList(); if (slots.Count == 0) return false; var setSlot = slots[_battle.Random.Next(slots.Count)];
        if (!_battle.TryPlacePassive("ai", setSlot.SlotIndex, card)) return false;
        if (!_aiDeck.SetPassive(card)) { _battle.RemovePassive(card); return false; }
        if (!setSlot.SetPassive(card)) { _battle.RemovePassive(card); _aiDeck.DiscardPlaced(card); return false; }
        AddLog($"[color=ff99aa]AI伏牌[/color] 敌方在 {setSlot.SlotIndex + 1} 号英雄槽背面设置了1张被动锦囊。"); await TriggerPassive(_allies, _deck, "PASSIVE_SET", new PassiveEventContext { EventKey = "PASSIVE_SET", SubjectCard = card, SubjectSlotIndex = setSlot.SlotIndex, SubjectOwnerId = "ai" }); RefreshEnemyHand(); return true; }
    ...
    private async Task UseStealCard(CardInstance card, bool byAi)
    {
        ...
        var counterSlot = FindCounterSlot(byAi); if (counterSlot != null) { await TriggerCounter(counterSlot, byAi); await Announce("「我觉得不行」发动：拿来主义被抵消"); (byAi ? _aiDeck : _deck).Discard(card); if (!byAi) _ap -= card.CurrentCost(); _pendingCard = null; _pendingCardTarget = -1; CallDeferred(MethodName.RefreshAll); return; }
    ...
    private void MarkDefeated()
    {
        ...
        foreach (var s in _allies)
            if (s.Unit is { Alive: false }) newlyDead.Add((s.Unit!, "player"));
        foreach (var s in _enemies)
            if (s.Unit is { Alive: false }) newlyDead.Add((s.Unit!, "ai"));
        ...
        foreach (var (deadUnit, side) in newlyDead)
        {
            var ctx = new PassiveEventContext { EventKey = "HERO_DIED", SubjectCard = null, SubjectOwnerId = side };
            if (side == "player")
                TriggerPassive(_enemies, _aiDeck, "HERO_DIED", ctx).Wait();
            else
                TriggerPassive(_allies, _deck, "HERO_DIED", ctx).Wait();
        }
    ...
    private int EffectiveCost(CardInstance c) => c.Definition.id.ToString() == _freeCardId ? 0 : c.Definition.cost_mode == "ALL_CURRENT" ? Math.Min(_ap, c.Definition.action_cost) : c.CurrentCost();
