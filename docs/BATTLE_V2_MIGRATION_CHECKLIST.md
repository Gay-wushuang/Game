...
| 2.4 | Turn End 剩余 AP 清零 | `EndTurn()` 已清零 | ✅ Done | `TrainingArena.cs:L194` | Phase 2 |
...
| 3.5 | 抽牌堆不足时洗回弃牌堆 | 未实现自动洗回；`DeckState` 无此逻辑 | ❌ Gap | `DeckState.cs` | Phase 2 |
...
| 5.4 | 普攻伤害最低 0 | `Math.Max(0, d.Hp - damage)` 已有 | ✅ Done | `TrainingArena.cs:L104` | Phase 4 |
...
| 6.6 | 持续型被动继续保留 | 由卡牌 `post_zone` 参数控制 EXILE vs 默认 Discard | ⚠️ Partial | `TrainingArena.cs:L242` | Phase 5 |
| 6.7 | 被动默认对敌方隐藏 | AI 侧：`FaceUp = false`（CardInstance 已支持）；玩家侧：当前直接显示 | ⚠️ Partial | `UnitSlot.cs`, `CardTile.cs` | Phase 5 |
...
| 8.3 | 弃牌堆洗回使用同一 RNG | 未实现弃牌堆洗回 | ❌ Gap | `DeckState.cs` | Phase 2 |
...
| 9.5 | UI 不保存权威状态 | UI `UnitSlot` 保存 `UnitState` 引用 | ❌ Gap | `UnitSlot.cs` | Phase 6 |
...
| 11.2 | 先锋→刺客→斥候→先锋 循环 | `BattleRules` 已实现 | ✅ Done | `BattleRules.cs:L6-11` | Done |
...
| 12.2 | 技能不消耗普攻次数 | 无 `HasAttackedThisTurn` 限制 | ✅ Done (默认) | `TrainingArena.cs` | Phase 4 |
